...
## Reglas de compra y stock
- Las órdenes se crean en estado `pending` con reserva por **24 horas** (`ORDER_HOLD_HOURS`).
- **No se descuenta stock al crear la orden**; se descuenta recién cuando el pago se confirma por webhook.
- Si pasan 24 horas sin pagar, la orden pendiente se elimina automáticamente.
- Tracking básico disponible en `GET /api/orders/{id}/tracking`.

## Super Admin inicial
En el primer arranque del backend se crea automáticamente (si no existe) un super admin:
- Nombre: `Rojas Augusto`
- Email login: `rojas.augusto@admin.local`
